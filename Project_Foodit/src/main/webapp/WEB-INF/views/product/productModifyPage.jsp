	...
	<form action="productModify" method="POST" enctype="multipart/form-data">
		<div class="pdDetailWrap">
			 	...
			 	<c:if test="${mDto.role == 'admin' }">	<!-- 관리자만 보이는 부분   -->
					...
					<button type="button" onclick="productRemove()">상품 삭제</button> 
					<button type="button" onclick="location.href='productList'">상품 목록</button>	
			 	</c:if>
						...
						<div class="pdRpriWrap">
							<dl class="priceTxt">
								...
								<dd>
									...
									<span class="pdPrice">
										<dt> ,(콤마)사용금지</dt>
										<input type="text" name="product_price" value="${productDto.product_price}"/>
									</span>
								</dd>
							</dl>
						</div>
					 	<dt>상품 이미지 등록(-info) <input type="file" name="file_2"/></dt>
						<dt>	상품 썸네일 등록 <input type="file" name="file_2"/></dt>				  
					</div>
				</div>
