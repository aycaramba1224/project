...
public class ProductListOrder2Command implements ProductCommand {
	...
	public void execute(SqlSession sqlSession, Model model) {
		...
		// ◀ 1 2 3 ▶ 생성 (pagingView)
		String pagingView = Paging.getPaging("productListOrder2", nowPage, recordPerPage, totalRecord);
		
		model.addAttribute("currentPage", currentPage);
		model.addAttribute("pagingView", pagingView);	
		model.addAttribute("productListOrder2", productDao.productListOrder2(begin, end));	
		model.addAttribute("productListOrder2Size", productDao.productListOrder2(begin, end).size());
	}
