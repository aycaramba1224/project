...
public class ProductListOrder4Command implements ProductCommand {
	...
	public void execute(SqlSession sqlSession, Model model) {
		...
		// ◀ 1 2 3 ▶ 생성 (pagingView)
		String pagingView = Paging.getPaging("productListOrder4", nowPage, recordPerPage, totalRecord);
		
		model.addAttribute("currentPage", currentPage);
		model.addAttribute("pagingView", pagingView);	
		model.addAttribute("productListOrder4", productDao.productListOrder4(begin, end));	
		model.addAttribute("productListOrder4Size", productDao.productListOrder4(begin, end).size());
	}
