...
public class FaqSearchCommand implements FaqCommand {
	...
	public void execute(SqlSession sqlSession, Model model) {
      ...
      // 현재 페이지 번호 구하기 (파라미터로 전달)
   		...
   		int totalRecord = fDao.faqSearchTotalCount("%" + faq_content + "%");
		String pagingView = Paging2.getPaging("faqSearchResult?faq_content=" + faq_content, nowPage, recordPerPage, totalRecord);
   		
   		
   		
   		ArrayList<FaqDto> list = fDao.faqSearchResult( "%" + faq_content + "%", begin, end);
