...
public class ProductListOrder3Command implements ProductCommand {
	...
	public void execute(SqlSession sqlSession, Model model) {
		...
		// ◀ 1 2 3 ▶ 생성 (pagingView)
		String pagingView = Paging.getPaging("productListOrder3", nowPage, recordPerPage, totalRecord);
		
		model.addAttribute("currentPage", currentPage);
		model.addAttribute("pagingView", pagingView);	
		model.addAttribute("productListOrder3", productDao.productListOrder3(begin, end));	
		model.addAttribute("productListOrder3Size", productDao.productListOrder3(begin, end).size());
	}
